...
describe("Dashboard", () => {
  describe("Login test", () => {
    ...
    it("掃描狀況大表，獎項已抽出數字是否顯示？", () => {
      cy.visit(`${DEV}login?token=${TOKEN}`);
      cy.contains("開獎 1").should("be.visible");
      cy.contains("水杯⼩").siblings('.style_count__1xbVh').get('span').contains('0').should("be.visible");
      cy.contains("⽔杯⼤").siblings('.style_count__1xbVh').get('span').contains('0').should("be.visible");
      cy.contains("野餐墊").siblings('.style_count__1xbVh').get('span').contains('0').should("be.visible");
      cy.contains("冰 桶").siblings('.style_count__1xbVh').get('span').contains('1').should("be.visible");
      cy.contains("行李箱").siblings('.style_count__1xbVh').get('span').contains('0').should("be.visible");
    });
